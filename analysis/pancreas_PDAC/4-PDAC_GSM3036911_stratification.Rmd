...
st_list <- sapply(pdac_smpl, USE.NAMES = TRUE, function(geo) {
  ...
  st_se <- RunPCA(st_se, assay = "SCT", verbose = FALSE)
  st_se <- FindNeighbors(st_se, reduction = "pca", dims = 1:40)
  st_se <- FindClusters(st_se, verbose = FALSE)
  st_se <- RunUMAP(st_se, reduction = "pca", dims = 1:40)
...
Spaniel::spanielPlot(object = st_se,
                     grob = st_se@images[[1]], 
                     plotType = "Cluster", 
                     clusterRes = "SCT_snn_res.0.8") +
  theme_void() +
  coord_fixed(1) +
...
tmp_plt_2 <- ggplot2::ggplot(data_df,
...
```{r}
decon_mtrx_immune <- immune_decon_mtrx_ls[[1]]
...
hm_tmp <- t(hm_df_2) %>%
  ...
  dplyr::mutate(proportion_mod = if_else(status == "Adj. Pval", 0, proportion),
                plt_name = as.character(plt_name),
                plt_name = if_else(plt_name == "Macro_1", "Macrophage", plt_name),
                plt_name = if_else(plt_name == "Mono", "Monocyte", plt_name),
                pval = if_else(status == "Adj. Pval", proportion, NA_real_)) %>% 
  ggplot(aes(x = status, 
             y = plt_name, 
             fill= proportion_mod)) + 
  geom_tile(aes(alpha = proportion_mod)) +
  geom_text(aes(label = round(pval, 2)),
            show_guide  = FALSE) +
  theme_classic() +
  labs(title = "Cell type enrichment per region",
       y = "",
       x = "",
       fill = "Proportion",
       alpha = "Proportion") +
...
ggpubr::ggexport(plotlist = list(hm_tmp),
                 filename = sprintf("%s/%s/heatmap_pvalue_GSM3036911_status_2_territories.jpg",
                                    an_pdac, plt_dir),
                 ncol = 2,
                 width = 3000, 
                 height = 4000, 
...
ggpubr::ggexport(plotlist = list(tmp_plt_2),
                 filename = sprintf("%s/%s/GSM3036911_status_2_territories.jpg",
                                    an_pdac, plt_dir),
                 ncol = 2,
                 width = 4000, 
                 height = 4000, 
...
bplt <- decon_mtrx_long %>%
  ...
  ggpubr::geom_signif(
    data = anno_df %>% filter(p.adj < 0.05 & plt_name != "MAST"), 
    aes(xmin = group1, 
        xmax = group2, 
...
ggpubr::ggexport(plotlist = list(bplt),
                 filename = sprintf("%s/%s/boxplot_GSM3036911_status_2_territories_filt.pdf",an_pdac, plt_dir),
                 width = 16,
                 height = 12,
                 res = 600)

ggpubr::ggexport(plotlist = list(bplt),
                 filename = sprintf("%s/%s/boxplot_GSM3036911_status_2_territories_filt.jpg",an_pdac, plt_dir),
                 width = 6000,
                 height = 4000,
                 res = 400)
...
ggpubr::ggexport(plotlist = list(bplt),
                 filename = sprintf("%s/%s/boxplot_GSM3036911_status_2_territories.jpg",an_pdac, plt_dir),
                 width = 6000,
                 height = 4000,
                 res = 400)
...
ggpubr::ggexport(plotlist = list(strat_3_plt +
                                   guides(color = guide_legend(override.aes = list(size = 10))) +
                                   guides(fill = guide_legend(override.aes = list(size = 10))) +
                                   theme(legend.title = element_text(size = 15, face = "bold"),
                                         legend.text = element_text(size = 12))),
                 filename = sprintf("%s/%s/GSM3036911_status_3_territories.jpeg", 
                                      an_pdac, plt_dir),
...
decon_mtrx_long <- decon_df_immune %>% 
  ...
  dplyr::left_join(pdac_plt_names, by = c("cell_type" = "df_name")) %>% 
  dplyr::mutate(plt_name = as.character(plt_name)) %>% 
  dplyr::filter(cell_type %in% c("B.cell", "CD4.CM", "CD4.naive.like", "CD8.effector.exhausted", "CD8.proliferative", "Plasma.B"))
```
...
```{r}
p <- ggpubr::ggboxplot(decon_mtrx_long, x = "status_3_territories", y = "proportion",
                  color = "status_3_territories", fill = "status_3_territories",
                  add = "jitter", alpha = 0.6,
                  facet.by = "plt_name", scales = "free", 
                  repel = TRUE, palette = c("#00A087FF", "#4DBBD5FF", "#E64B35FF"))

# Create a box plot and add the p-value
# p <- ggboxplot(
#   decon_mtrx_long, x = "status_3_territories", y = "proportion",
#   color = "status_3_territories", palette = "npg",
#   facet.by = "plt_name", ylim = c(0, 1)
# )
...
bplt
ggpubr::ggexport(plotlist = list(bplt),
                 filename = sprintf("%s/%s/boxplot_GSM3036911_status_3_territories.pdf",an_pdac, plt_dir),
                 width = 32,
                 height = 24,
                 res = 600)

ggpubr::ggexport(plotlist = list(bplt),
                 filename = sprintf("%s/%s/boxplot_GSM3036911_status_3_territories.jpeg",an_pdac, plt_dir),
                 width = 6000,
                 height = 4000,
                 res = 400)
