...
ct_arrange <- lapply(cell_types, function(ct) {
  ...
  ggarrange(plotlist = ., ncol = 4, nrow = 5, align = "hv")

ggpubr::ggexport(plotlist = ct_arrange,
                   filename = sprintf("%s/%s/Supplementary_Figure_AAA_all_cell_types_grid_%s_%s.pdf", 
...
### Topic signature reproducibility
...
topic_plt_arr <- lapply(seq_len(10), function(i) {
  ...
  tmp_plt <- topic_profile_plts[[2]] + 
    labs(title = sprintf("Topic signatures: Iteration %s", i)) +
    theme(
...
ggpubr::ggexport(plotlist = list(topic_plt_arr),
