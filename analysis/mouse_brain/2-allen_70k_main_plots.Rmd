...
```{r}
col_df <- data.frame(cell_types = c(cell_types, "L2.3"),
...
ct_dict[["CA1sp"]][["plot_name"]] <- "Cornu Ammonis 1"
...
ct_dict[["CA3sp"]][["plot_name"]] <- "Cornu Ammonis 3"
...
plot_ls <- lapply(ct_ls, function(ct) {
  ...
  ggpubr::ggexport(plotlist = list(final_arr),
                         filename = sprintf("analysis/mouse_brain/Fig_%s_arrange.pdf", nam), 
                         width = 7.91, 
                         height = 9.38, 
                         res = 600)
...
```{r}
cowplot::plot_grid(plotlist = list(plot_ls[[1]], NULL, plot_ls[[2]],
                                   NULL, NULL, NULL,
...
### Min contribution evaluation
```{r}
min_cont_df <- readRDS(file = sprintf("%s/%s/min_cont_df_%s.RDS", an_mouse, robj_dir, id_nmf))
min_cont_long_df <- min_cont_df %>% tidyr::pivot_longer(cols = c("Accuracy", 
                                             "Specificity", "Sensitivity", "Precision", 
                                             "F1 score"),
                                    names_to = "Metric", 
                                    values_to = "Value") %>% 
  mutate(min_cont = factor(x = min_cont, 
                           levels = as.character(sort(as.numeric(unique(min_cont))))))
```
...
best_min_cont <- min_cont_df[min_cont_df$`F1 score` == max(min_cont_df$`F1 score`), "min_cont"]
p1 <- min_cont_long_df %>%
  mutate(best = if_else(min_cont == best_min_cont, TRUE, FALSE)) %>% 
  ggplot() +
  geom_point(aes(x = min_cont, y = Value, colour = best)) +
  facet_wrap(Metric~.) +
  ylim(0, 1) +
  theme_classic() +
  my_theme +
  theme(legend.position = "none") +
  labs(x = "min_cont", y = "", colour = "") +
  scale_color_manual(values = c(col_high, col_low))

p1
```

Visualize Jesnen-Shannon divergence distance
```{r}
p2 <- min_cont_df %>% 
  ggplot(aes(x = reorder(min_cont, -JSD_50), y = JSD_50)) +
  geom_errorbar(aes(ymin = JSD_25, ymax = JSD_75), width = .3, size = 1) +
  geom_point(aes(size = time_mins)) +
  my_theme +
  labs(x = "", y = "")
...
p3 <- tech_raw_ds %>%
tidyr::pivot_longer(cols = c("JSD_25", "JSD_50", "JSD_75"),
                names_to = "quantile", values_to = "value") %>%
dplyr::mutate(
  value = as.numeric(value),
  tech = factor(tech, levels = levels(reorder(tech_proc$tech, -tech_proc$JSD_50)))) %>% 
ggplot(aes(x = tech, y = value, color = quantile)) +
geom_point() +
my_theme
p3
...
# Least-squares residuals plot
We want to check the least square residuals and see if we are better at predicting in some regions
```{r}
brain[["res_ss"]] <- decon_mtrx[, "res_ss"]
SpatialFeaturePlot(object = brain, features = "res_ss")
which(is.na(decon_mtrx[, "res_ss"]))
```
